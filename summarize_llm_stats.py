...
# summarize_llm_stats.py (v2)
# Robust daily summarizer for https://llm-stats.com data (from GitHub repo).
# - Uses Git Trees API (1 call) to list all files recursively.
# - Supports optional GITHUB_TOKEN/GH_TOKEN to avoid rate limits.
# - Sends concise Telegram summary grouped by Coding / Math / Reasoning.
...
USER_AGENT = "llm-stats-telegram-summary/2.0 (+https://llm-stats.com)"
...
def _get(url: str, retry: int = 4, timeout: int = 30, headers: Dict[str, str] = None):
    for i in range(retry):
        ...
        if r.status_code in (429, 500, 502, 503, 504):
            time.sleep(2 ** i)
            continue
        # For 403 rate limit, backoff then retry once with delay

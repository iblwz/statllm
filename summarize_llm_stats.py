# summarize_llm_stats.py (v4)
# Adds fallback: parse README.md leaderboard table if per-model JSONs lack benchmark keys.
...
README_RAW = RAW_BASE + "README.md"

USER_AGENT = "llm-stats-telegram-summary/4.0 (+https://llm-stats.com)"

CATEGORIES = {
    "Coding": ["humaneval", "livecodebench", "mbpp", "apps", "leetcode", "codeforces"],
    "Math": ["aime", "math", "gsm8k", "amc", "olympiad"],
    "Reasoning": ["gpqa", "mmlu-pro", "mmlu", "mmmu", "ifeval", "arc-c", "hellaswag"],
}
...
def extract_model_scores(model_json: Dict[str, Any]) -> Dict[str, float]:
    ...
    if not containers:
        # some files may just be a flat structure
...
def parse_readme_table(md: str) -> List[Tuple[str, Dict[str, float]]]:
    lines = [ln.strip() for ln in md.splitlines()]
    # find header line that includes key columns
    start = -1
    for i, ln in enumerate(lines):
        low = ln.lower()
        if ("name" in low) and ("humaneval" in low or "gpqa" in low or "aime" in low) and ("|" in ln):
            start = i
            break
    if start == -1:
        return []
    # expect header, separator, then rows beginning with |
    header = [c.strip().lower() for c in lines[start].split("|")]
    # map column indices we care about
    wanted = ["name", "humaneval", "math", "aime 2024", "gpqa", "mmlu", "mmlu-pro", "mmmu"]
    idx = {w: (header.index(w) if w in header else -1) for w in wanted}
    ...
    for ln in lines[start+2:]:
        ...
        name = cols[idx["name"]] if idx["name"] >= 0 else cols[1]
        ...
        def put(colname, key):
            ...
            if j >= 0:
                v = _to_float(cols[j])
                if v == v:
                    scores[key] = _maybe_percent_to_unit(v)
        put("humaneval","humaneval.pass@1")
        put("math","math")
        put("aime 2024","aime 2024")
        put("gpqa","gpqa")
        put("mmlu","mmlu")
        put("mmlu-pro","mmlu-pro")
        put("mmmu","mmmu")
        out.append((name, scores))
    return out

def build_message(tops: Dict[str, List[Tuple[str, float]]], model_count: int, scanned: int, fallback_used: bool) -> str:
    lines = []
    ...
    lines.append(f"_scanned files: {scanned}{' • README fallback' if fallback_used else ''}_")
    return "\n".join(lines)
...
def main():
    ...
    # Pick tops from JSON data first
    tops = pick_tops(all_models, top_n=5)
    fallback_used = False

    if not any(tops.get(cat) for cat in ("Coding","Math","Reasoning")):
        # Fallback to README parsing
        try:
            rr = _get(README_RAW, headers={"Accept": "text/plain"})
            fallback_models = parse_readme_table(rr.text)
            if fallback_models:
                fallback_used = True
                all_models.extend(fallback_models)
                tops = pick_tops(all_models, top_n=5)
        except Exception as e:
            print(f"[warn] README fallback failed: {e}", file=sys.stderr)

    msg = build_message(tops, model_count=len(all_models), scanned=scanned, fallback_used=fallback_used)
    send_telegram(msg)
    print(f"Sent summary to Telegram. files_scanned={scanned}, models={len(all_models)}, fallback={fallback_used}")
